...
Concrètement, les mots de passe sont enregistrés dans une base de données qui est [chiffrée](https://fr.wikipedia.org/wiki/Chiffrement). Pour accéder aux mots de passe, il est nécessaire de posséder la clé de déchiffrement. Cette clé est représentée par un unique mot de passe : votre mot de passe maître.
...
Un·e attaquant·e a pu accéder à l'ordinateur d'un·e ingénieur·e logiciel de LastPass. Depuis cet ordinateur, iel a pu accéder à des informations confidentielles appartenant à LastPass mais aucune information de client·es.
Les données récoltées par l'attaquant·e durant cet incident lui ont permis de lancer une seconde attaque.
